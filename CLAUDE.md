...
**Context files:** Three Markdown files (`IDENTITY.md`, `SOUL.md`, `USER.md`) live in `workspace/context/`. They are prepended to the system prompt only when enabled by `personality.identity.enabled`, `personality.soul.enabled`, and `personality.user.enabled`. The agent can read and write them via built-in tools. A `FileSystemWatcher` invalidates the cache when any file changes. Monitor combined token count; warn the user if they approach the limit.

**Conversation loop (ARIA.Agent):** The `ConversationLoop` runs a tool-dispatch loop capped at `MaxToolCallIterations` (default 10) to prevent infinite cycles. Flow: inject enabled context files → load recent history when `personality.memory.enabled` is true (N turns from SQLite) → call LLM → if tool calls returned, execute each via `ISkillRegistry`, append results, loop → persist assistant turn → return response text.
